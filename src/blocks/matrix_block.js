...
import { valuePkg, makeJSArray } from './blockUtils';
 ...
 * @package
 */
const NUMPY_SQUARE_MATRIX_MUTATOR_MIXIN = {
  /**
  ...
  updateShape_: function () {
    ...
    while (this.getInput('line_' + i)) {
      matrixData.push(this.getInput('line_' + i).fieldRow.slice(1).map(item => item.value_));
