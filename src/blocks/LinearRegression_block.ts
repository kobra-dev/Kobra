  ...
  Blockly.defineBlocksWithJsonArray([
    {
      type: 'linr_create',
      message0:
        'linear regression model: %1 Training data x: %2 Training data y: %3',
      args0: [
        {
          type: 'input_dummy'
        },
  ...
  return [
    {
      ...
      f: (block) =>
        valuePkg(
          constructCodeFromParams(block, 'linr_create', 'X_VAL', 'Y_VAL')
        )
    },
    {
      block: 'linr_fit',
      f: (block) =>
        statementPkg(constructCodeFromParams(block, 'linr_fit', 'VALUE'))
    },
    {
      block: 'linr_predict',
      f: (block) =>
