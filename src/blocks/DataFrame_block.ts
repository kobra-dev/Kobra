...
import { valuePkg, constructCodeFromParams, ArgType, statementPkg, BlocklyJSDef } from './blockUtils';
...
export function df_init_blocks() : BlocklyJSDef[] {
    ...
    return [
        ...
        {
            block: 'df_create',
            f: block => valuePkg(constructCodeFromParams(block, "df_create", 'HEAD_VAL', 'DATA_VAL'))
        },
        {
            block: 'df_create_from_csv',
            f: block => valuePkg(constructCodeFromParams(block, "df_create_from_csv", {
                type: ArgType.Field,
                arg: 'CSV_DROPDOWN'
            }))
        },
        {
            block: 'df_transpose',
            f: block => statementPkg(constructCodeFromParams(block, "df_transpose", {
        ...
        {
            block: 'df_loc',
            f: block => statementPkg(constructCodeFromParams(block, "df_loc", {
                type: ArgType.Variable,
                arg: 'DF_VAL'
            }, {
                type: ArgType.Value,
